...
#else
  (void)arg; /* LV_TICK_CUSTOM==1 uses LV_TICK_CUSTOM_SYS_TIME_EXPR (millis()),
                no need to call lv_tick_inc */
#endif
}
...
void example_lvgl_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area,
...
void LVGL_Init(void) {
  ...
  lv_disp_drv_init(&disp_drv); // Create a new screen object and initialize the
                               // associated device
  ...
  disp_drv.flush_cb =
      example_lvgl_flush_cb; // Function : copy a buffer's content to a specific
                             // area of the display
  disp_drv.drv_update_cb =
      example_lvgl_port_update_callback; // Function : Rotate display and touch,
                                         // when rotated screen in LVGL. Called
