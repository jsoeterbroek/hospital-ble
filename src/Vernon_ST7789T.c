...
static esp_err_t panel_st7789t_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
                                           int y_start, int x_end, int y_end,
                                           const void *color_data);
static esp_err_t panel_st7789t_invert_color(esp_lcd_panel_t *panel,
                                            bool invert_color_data);
static esp_err_t panel_st7789t_mirror(esp_lcd_panel_t *panel, bool mirror_x,
                                      bool mirror_y);
static esp_err_t panel_st7789t_swap_xy(esp_lcd_panel_t *panel, bool swap_axes);
static esp_err_t panel_st7789t_set_gap(esp_lcd_panel_t *panel, int x_gap,
                                       int y_gap);
...
esp_err_t esp_lcd_new_panel_st7789t(
  ...
  ESP_GOTO_ON_FALSE(st7789t, ESP_ERR_NO_MEM, err, TAG,
                    "no mem for st7789t panel");
...
static esp_err_t panel_st7789t_init(esp_lcd_panel_t *panel) {
  ...
  // {st7789t->madctl_val,}, 1); esp_lcd_panel_io_tx_param(io, LCD_CMD_COLMOD,
  // (uint8_t[]) {st7789t->colmod_cal,}, 1);

  /* Memory Data Access Control, MX=MV=1, MY=ML=MH=0, RGB=0 */
  esp_lcd_panel_io_tx_param(io, 0x36, (uint8_t[]){0x00},
                            1); // 0x36: 接口像素格式 X镜像，Y镜像
  ...
  esp_lcd_panel_io_tx_param(io, 0xE0,
                            (uint8_t[]){0xD0, 0x0D, 0x14, 0x0D, 0x0D, 0x09,
                                        0x38, 0x44, 0x4E, 0x3A, 0x17, 0x18,
                                        0x2F, 0x30},
                            14);
  /* Negative Voltage Gamma Control */
  esp_lcd_panel_io_tx_param(io, 0xE1,
                            (uint8_t[]){0xD0, 0x09, 0x0F, 0x08, 0x07, 0x14,
                                        0x37, 0x44, 0x4D, 0x38, 0x15, 0x16,
                                        0x2C, 0x2E},
                            14);
  /* Sleep Out */
...
static esp_err_t panel_st7789t_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
  ...
  size_t len =
      (x_end - x_start) * (y_end - y_start) * st7789t->fb_bits_per_pixel / 8;
...
static esp_err_t panel_st7789t_disp_on_off(esp_lcd_panel_t *panel,
                                           bool on_off) {
  st7789t_panel_t *st7789t = __containerof(panel, st7789t_panel_t, base);
  esp_lcd_panel_io_handle_t io = st7789t->io;
  int command = 0;
